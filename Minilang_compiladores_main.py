...
def t_INDENT(t):
    ...
    if not inicio_linea:
        return # Si no es inicio de línea, ignora los espacios (comportamiento normal)
...
# ASEGÚRATE de que esta función se llame exactamente p_sentencia 
# y que incluya todas tus reglas
def p_sentencia(p):
...
if __name__ == "__main__":
    # Asegúrate de que la carpeta se llame exactamente 'pruebas'
    ...
    try:
        with open(archivo_test, "r", encoding="utf-8") as f:
            ...
            data += '\n' 
            # Reiniciamos las variables globales para el lexer
            pila_indentacion = [0]
            dedents_pendientes = []
            inicio_linea = True
            linea_token = False
